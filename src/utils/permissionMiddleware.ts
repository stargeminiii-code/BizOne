...
export function canAccessView(user: UserAccount | null | undefined, view: ViewMode): boolean {
  ...
  if (user.role === 'super_admin' || user.id === 'usr-admin-ductang' || user.phone === '0968994439') {
    return true;
  }
...
export function hasPermission(
  ...
  if (!user) return false;
  if (user.role === 'super_admin' || user.id === 'usr-admin-ductang' || user.phone === '0968994439') return true;
